...
class BankController extends CommonHomeController {
    // 银行卡浏览页面
    public function index(){
      $bank=M('bank');
      $user_id=$_SESSION['uid'];
      $list=$bank->where('uid='.$user_id)->select();
      // $number=count($list);
    ...
    public function add(){
      if(IS_POST){
        ...
        if($bank->create()){
            ...
            $card_total_name=bankInfo($card,$bankList);
            $arr=explode('-',$card_total_name);
            //拆分结束
            $user_id=$_SESSION['uid'];
            $bank->cardnum=$cardnum;
            $bank->idcard=$idcard;
            $bank->name=$name;
            $bank->uid=$user_id;
            $bank->bankname=$arr[0];
            $bank->cardname=$arr[1];
            $bank->cardtype=$arr[2];
            $bank->create_time=get_date();
            $bank->status=1;
            file_put_contents('text.txt',$user_id.$arr[0].$arr[1].$arr[2]);
            $bank->add(); // 写入数据到数据库
