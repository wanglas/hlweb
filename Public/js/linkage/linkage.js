	...
	jQuery.fn.extend({
	    linkage:function(options){    //菜单联动
			...
			for(var i=0;i<obj.set.length;i++){
				...
				if(data[obj.set[i]]==null||data[obj.set[i]]['child']==null)		//没有当前级数据或没有子数据则跳出循环
					break;
				data=data[obj.set[i]]['child']!=null?data[obj.set[i]]['child']:null;
			}
			...
			function choose(i){
				...
				var l= 0; 
				while(l<i) { 
					if(data[level[l][0]]!=null&&data[level[l][0]]['child']!=null){
						data=data[level[l][0]]['child'];
						l++; 
					} else {
						return false;
					}
				...
				.children("span").click(function(){
					...
					if(data[level[i][0]]!=null&&data[level[l][0]]['child']!=null){
			...
			function searchKey(data,val,pid){
			  ...
			  if(!find){
			    for(var j=0;j<level.length;j++){
			       if(data[level[j]]!=undefined&&data[level[j]]["child"]!=undefined){
			          //console.log(level[j]);
			          //console.log(data[level[j]]);
			          searchKey(data[level[j]]["child"],val,level[j]);
			       }
			    }
			  }
			  //console.log(level);
